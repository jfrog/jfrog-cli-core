...
func (bpc *BuildPublishCommand) Run() error {
	...
	if err = recordCommandSummary(buildInfo, buildLink, bpc.serverDetails.Url); err != nil {
		return err
	}
...
func recordCommandSummary(buildInfo *buildinfo.BuildInfo, buildLink, serverUrl string) (err error) {
	if !commandsummary.ShouldRecordSummary() {
		return
	}
	buildInfo.BuildUrl = buildLink
	buildInfoSummary, err := commandsummary.New(commandssummaries.NewBuildInfo(serverUrl), "build-info")
	if err != nil {
		return
	}
