...
func (dc *DeleteCommand) GetPathsToDelete() (*content.ContentReader, error) {
	...
	servicesManager, err := utils.CreateServiceManager(serverDetails, dc.retries, dc.retryWaitTime, dc.DryRun())
	if err != nil {
		return nil, err
	}
...
func (dc *DeleteCommand) DeleteFiles(reader *content.ContentReader) (successCount, failedCount int, err error) {
	...
	servicesManager, err := utils.CreateDeleteServiceManager(serverDetails, dc.Threads(), dc.retries, dc.retryWaitTime, dc.DryRun())
	if err != nil {
		return 0, 0, err
	}
