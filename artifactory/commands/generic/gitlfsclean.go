...
func (glc *GitLfsCommand) Run() error {
	...
	servicesManager, err := utils.CreateServiceManager(serverDetails, glc.retries, glc.retryWaitTime, glc.DryRun())
	if err != nil {
		return err
	}
...
func (glc *GitLfsCommand) deleteLfsFilesFromArtifactory(deleteItems *content.ContentReader) error {
	...
	servicesManager, err := utils.CreateServiceManager(glc.serverDetails, glc.retries, glc.retryWaitTime, glc.DryRun())
	if err != nil {
		return err
	}
