...
func (uc *UploadCommand) upload() error {
	...
	servicesManager, err := utils.CreateUploadServiceManager(serverDetails, uc.uploadConfiguration.Threads, uc.retries, uc.retryWaitTime, uc.DryRun(), uc.progress)
	if err != nil {
		return err
	}
...
func (uc *UploadCommand) handleSyncDeletes(syncDeletesProp string) error {
	servicesManager, err := utils.CreateServiceManager(uc.serverDetails, uc.retries, uc.retryWaitTime, false)
	if err != nil {
		return err
	}
