...
func (f *filesDiffPhase) createDiffTimeFrameHandlerFunc(pcDetails *producerConsumerWrapper, uploadTokensChan chan string, delayHelper delayUploadHelper, errorsChannelMng *ErrorsChannelMng) diffTimeFrameHandlerFunc {
	return func(params timeFrameParams) parallel.TaskFunc {
		return func(threadId int) error {
			logMsgPrefix := clientUtils.GetLogMsgPrefix(threadId, false)
			return f.handleTimeFrameFilesDiff(pcDetails, params, logMsgPrefix, uploadTokensChan, delayHelper, errorsChannelMng)
		}
	}
}

func (f *filesDiffPhase) handleTimeFrameFilesDiff(pcDetails *producerConsumerWrapper, params timeFrameParams, logMsgPrefix string, uploadTokensChan chan string, delayHelper delayUploadHelper, errorsChannelMng *ErrorsChannelMng) error {
	...
	for {
		...
		shouldStop, err := uploadByChunks(files, uploadTokensChan, f.phaseBase, delayHelper, errorsChannelMng, pcDetails)
...
func (f *filesDiffPhase) handlePreviousUploadFailures() error {
	...
	manager := newTransferManager(f.phaseBase, getDelayUploadComparisonFunctions(f.repoSummary.PackageType))
	action := func(pcDetails *producerConsumerWrapper, uploadTokensChan chan string, delayHelper delayUploadHelper, errorsChannelMng *ErrorsChannelMng) error {
		return f.handleErrorsFiles(pcDetails, uploadTokensChan, delayHelper, errorsChannelMng)
	}
...
func (f *filesDiffPhase) handleErrorsFiles(pcDetails *producerConsumerWrapper, uploadTokensChan chan string, delayHelper delayUploadHelper, errorsChannelMng *ErrorsChannelMng) error {
	for _, path := range f.errorsFilesToHandle {
		...
		shouldStop, err := uploadByChunks(convertUploadStatusToFileRepresentation(failedFiles.Errors), uploadTokensChan, f.phaseBase, delayHelper, errorsChannelMng, pcDetails)
		if err != nil || shouldStop {
			return err
		}
