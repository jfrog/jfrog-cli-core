...
func (ftm *transferManager) doTransfer(isProducerConsumer bool, transferAction transferActionType) error {
	...
	go func() {
		defer runWaitGroup.Done()
		actionErr = transferAction(pcDetails, uploadTokensChan, delayUploadHelper{shouldDelayFunctions: ftm.delayUploadComparisonFunctions, delayedArtifactsChannel: delayedArtifactsChannel}, errorChannel)
		if !isProducerConsumer {
