...
type TransferProgressMng struct {
	...
	// A bar showing the number of visited directories
	visitedDirsBar *progressbar.TasksProgressBar
...
func initTransferProgressMng(allSourceLocalRepos []string, tdc *TransferFilesCommand, fileStatus int) error {
	...
	transfer.visitedDirsBar = transfer.transferMng.NewVisitedDirsBar()
...
func (t *TransferProgressMng) incNumberOfVisitedDirs() {
	if t.ShouldDisplay() {
		t.visitedDirsBar.SetGeneralProgressTotal(t.visitedDirsBar.GetTotal() + 1)
	}
}
...
func (t *TransferProgressMng) abortMetricsBars() {
	for _, barPtr := range []*progressbar.TasksProgressBar{t.runningTime, t.workingThreads, t.visitedDirsBar, t.delayedBar, t.errorBar, t.speedBar, t.timeEstBar, t.totalSize} {
		if barPtr != nil {
