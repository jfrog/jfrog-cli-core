...
func (pi *PipInstaller) Install(pythonExecutablePath string) error {
	pipenvInstallCmd := &python.PythonCmd{
		Executable:  pythonExecutablePath,
		Command:     "install",
		CommandArgs: append(pi.Args, "-i"),
	}
	...
	if pi.ShouldParseLogs {
		return pi.runPipInstallWithLogParsing(pipenvInstallCmd)
	}
...
func (pi *PipInstaller) runPipInstallWithLogParsing(pipInstallCmd *python.PythonCmd) error {
