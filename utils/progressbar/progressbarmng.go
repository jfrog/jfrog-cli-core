...
func (bm *ProgressBarMng) NewTasksWithHeadlineProg(totalTasks int64, headline string, spinner bool, color Color, transferType TransferJobType) *tasksWithHeadlineProg {
	bm.barsWg.Add(1)
	...
	if totalTasks == 0 {
		...
		prog.tasksProgressBar = bm.NewTasksProgressBar(totalTasks, color, transferType)
	}
...
func (bm *ProgressBarMng) NewTasksProgressBar(totalTasks int64, color Color, transferType TransferJobType) *tasksProgressBar {
	pb := &tasksProgressBar{}
	...
	pb.bar = bm.container.New(0,
		...
		mpb.AppendDecorators(
			decor.Name(" "+transferType.String()+": "),
