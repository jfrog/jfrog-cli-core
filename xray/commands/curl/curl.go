...
func (curlCmd *CurlCommand) Run() error {
	...
	uriIndex, targetUri, err := curlCmd.buildCommandUrl(curlCmd.serverDetails.XrayUrl)
	if err != nil {
		return err
	}
...
func (curlCmd *CurlCommand) buildCommandUrl(xrayUrl string) (uriIndex int, uriValue string, err error) {
	...
	// Attach Xray's url to the api.
	uriValue = xrayUrl + uriValue

	return
}

// Returns Xray details
